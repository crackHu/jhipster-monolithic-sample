...
const apiUrl = SERVER_API_URL + '/api/companies';
const apiSearchUrl = SERVER_API_URL + '/api/_search/companies';

// Actions

export const getSearchEntities: ICrudSearchAction<ICompany> = query => ({
  type: ACTION_TYPES.SEARCH_COMPANIES,
  payload: axios.get(`${apiSearchUrl}?query=` + query) as Promise<ICompany>
});
...
export const getEntities: ICrudGetAllAction<ICompany> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`) as Promise<ICompany>
  };
};
...
export const getEntity: ICrudGetAction<ICompany> = id => {
  ...
  return {
    ...
    payload: axios.get(requestUrl) as Promise<ICompany>
  };
};
