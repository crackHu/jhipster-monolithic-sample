...
const apiSearchUrl = SERVER_API_URL + '/api/_search/orders';

// Actions

export const getSearchEntities: ICrudSearchAction<IOrders> = query => ({
  type: ACTION_TYPES.SEARCH_ORDERS,
  payload: axios.get(`${apiSearchUrl}?query=` + query) as Promise<IOrders>
});
...
export const getEntities: ICrudGetAllAction<IOrders> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`) as Promise<IOrders>
  };
};
...
export const getEntity: ICrudGetAction<IOrders> = id => {
  ...
  return {
    ...
    payload: axios.get(requestUrl) as Promise<IOrders>
  };
};
